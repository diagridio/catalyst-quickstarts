...
/// <summary>
/// The two routes this quickstart exposes, kept out of Program.cs so the unit tests drive the app's
/// own handlers. Both read the verified caller with <c>GetVerifiedUser()</c>, which is trustworthy
/// because <c>OAuthMiddleware</c> refuses a tokenless request first.
/// </summary>
public sealed class IdentityEndpoints
{
    ...
    /// <summary>Who Catalyst says is calling. No model turn, so the 401/200 contrast is free.</summary>
    ...
    /// <summary>Runs the agent as the verified caller.</summary>
        ...
        CancellationToken cancellationToken)
    {
        ...
        using (body)
        {
            // Read defensively, so a body that is valid JSON of the wrong shape is a 400 with a
            // message saying so rather than a framework validation page.
        ...
        // Validated after authentication: the same body from an anonymous caller is still a 401.
    ...
    /// <summary>
    /// The verified caller, as JSON. Claim NAMES only, never values: <c>user.Claims</c> is a real
    /// person's decoded credential, and echoing it back would leak whatever is in it.
    /// </summary>
    ...
    /// <summary>This quickstart's own failure body, not an SDK error code.</summary>
