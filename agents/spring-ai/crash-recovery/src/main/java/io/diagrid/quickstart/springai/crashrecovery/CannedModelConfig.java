 ...
 *
 * <p>Kept out of {@link CrashRecoveryAgentConfig} on purpose: that class defines the agent and is
...
public class CannedModelConfig {
   ...
   * {@code equalsIgnoreCase}. So this must ignore case too. An {@code equals} comparison made
   * {@code DIAGRID_QUICKSTART_MODEL=OpenAI} satisfy <em>both</em> conditions, and two {@code
   * ChatModel} beans is fatal: three unqualified injection points fail with
   * {@code NoUniqueBeanDefinitionException} naming neither the variable the reader set nor the one
   * they meant.
   *
   ...
   * variable's value becomes SpEL source: a value containing a quote failed the context with a
   * parse error, which is the same class of confusing startup failure this condition exists to
   * avoid. Here the value is read as data and never parsed.
   *
   * <p>The two conditions are now exhaustive and mutually exclusive, so exactly one {@code
   * ChatModel} bean always exists for any value. {@code OpenAI} and {@code OPENAI} mean OpenAI,
   * matching what Spring AI itself would have done with them; anything else lands on the canned
   * model with a log line naming the variable.
   */
