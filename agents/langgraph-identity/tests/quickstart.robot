...
Python Langgraph Identity Quickstart
    ...
    Build Quickstart            ${qs}
    # README "## Run with Catalyst" step 2, run verbatim.
    ...
    # oversight: with require_auth=True the middleware wraps every route and
    # answers 401 on all of them, and the shipped OAuthConfig has no
    # exclude_paths, so no path can produce the 200 this keyword polls for. That
    # is also why the quickstart exposes no health route and why its dev config
    # sets enableAppHealthCheck: false. See HEALTH_PROBES in the data module.
    ...
    FOR    ${request}    IN    @{REQUESTS}
        ...
        # This suite is the first with a documented GET, so it branches instead of
