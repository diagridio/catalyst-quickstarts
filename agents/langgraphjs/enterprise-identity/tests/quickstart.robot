*** Comments ***
End-to-end test for the agents/langgraphjs/enterprise-identity quickstart
(javascript only: this quickstart has one implementation). The Python sibling at
agents/langgraph/enterprise-identity has its own suite and its own data module.

Mirrors agents/langgraphjs/enterprise-identity/README.md: "## Setup" installs,
...
commands succeed, the app connects through the dev tunnel, express serves on the
documented port, and an unauthenticated request to either documented route is
refused 401 with the exact body `{"error": "oauth.missing_token"}` -- the
shipped middleware's missing-token path (@diagrid/agent-core 0.2.0,
`oauthMiddleware` in identity/express.ts over `authenticate` in
identity/authenticate.ts).
...
The request loop below branches on ${request}[method], because the README's
primary fail-closed example is a GET. Same shape as the Python sibling's suite;
the other agent suites are POST-only and say so in their guard.

Run it:
  export DIAGRID_API_KEY=...
  eval "$(bash tools/qs-tester/ci/project-name.sh agents-enterprise-identity-js | grep '^PROJECT=')"
  bash tools/qs-tester/ci/login.sh
  cd tools/qs-tester
  uv run robot --variable PROJECT:$PROJECT --outputdir results/agents-enterprise-identity-js \
    ../../agents/langgraphjs/enterprise-identity/tests/quickstart.robot
  bash ci/teardown-project.sh "$PROJECT"

*** Settings ***
# Four levels up: this quickstart lives at
# agents/langgraphjs/enterprise-identity/tests/, two directory levels below
# agents/, like agents/spring-ai/event-planner.
...
Variables       ../../../../tools/qs-tester/variables/agents_enterprise_identity_javascript.py
Library         ../../../../tools/qs-tester/variables/agents_enterprise_identity_javascript.py
Library         Collections
...
*** Test Cases ***
Javascript Langgraph Identity Quickstart
    [Tags]    javascript    enterprise-identity    agents
    ${qs}=      Get Quickstart
    ${log}=     Suite Log File    agents-enterprise-identity-js    javascript
    ...
    FOR    ${secret}    IN    @{qs}[secrets]
        Require Env Var    ${secret}    agents/langgraphjs/enterprise-identity
    END
    ...
    # oversight: with requireAuth defaulting to true the middleware is installed
    # with `app.use` and answers 401 on every route, and the shipped OAuthConfig
    # has no per-path exclusion, so no path can produce the 200 this keyword
    # polls for. That is also why the quickstart exposes no health route and why
    ...
    FOR    ${request}    IN    @{REQUESTS}
        ...
        # Branches rather than asserting POST, because the README's primary
        # fail-closed example is a GET. An unrecognised method fails loudly
        # rather than being skipped: a request the loop quietly ignores is
        # coverage that reads as green and asserts nothing.
