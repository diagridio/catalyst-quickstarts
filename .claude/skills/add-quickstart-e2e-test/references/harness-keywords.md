...
| Keyword | Signature | Use it for |
|---|---|---|
| `Start Quickstart` | `${qs}  ${project}  ${logfile}` | Substitute `{project}` into `${qs}[run]`, record `${qs}[connected_apps]` (as a test variable, so `Stop Quickstart` can see it from teardown) for release later, and launch the command in the background under alias `apps`. |
...
| `Wait Until Apps Connected` | `${qs}  ${logfile}` | Wait for `Connected App ID "<id>" to http://localhost:<port>` per entry in `${qs}[connected_apps]`. Does nothing (empty list) for apps with `appPort` 0 or unset. Matches on the full `http://` line; a schemeless match would wait out the whole timeout silently. **Agent-family suites gate triggering on this too**: all three call it before their `READY_MARKERS` loop, `Wait Until Apps Healthy` and `Wait Until Catalyst Attached`, and for `agents/spring-ai/event-planner` — whose `READY_MARKERS` and `HEALTH_PROBES` are both empty — it is the *only* one of those that does anything. Confirmed against the 2026-08-27 live run: `diagrid dev run` does print this line for an agent-framework app. It means the local dev tunnel is up, not that Catalyst can route the app's workflow calls — that is `Wait Until Catalyst Attached`. Bounded by `${CONNECT_TIMEOUT}` (default 180s), NOT `${READINESS_TIMEOUT}`: it takes 32-36s against real Catalyst while the waits after it take about five seconds, so a mutation run that shortens `${READINESS_TIMEOUT}` must not shorten this one. |
| `Stop Quickstart` | `${project}` | The Ctrl+C equivalent: stops the `apps` process tree, then calls `Release App Connection` for every app `Start Quickstart` recorded in `${qs}[connected_apps]`. Safe to call when the process is already gone (wraps `Stop Process Tree` in `Run Keyword And Ignore Error` itself). |
| `Release App Connection` | `${app_id}  ${project}` | `diagrid dev stop --app-id ${app_id} --project ${project}` for one app ID, asserting the CLI's output actually says `disconnected` — `--project` alone or `-f <dev-config>` both exit 0 while doing nothing (v1.36.0), so the assertion is load-bearing, not decorative. Called once per `${qs}[connected_apps]` entry by `Stop Quickstart`; not usually called directly. |
...
| `Wait Until Ready Marker` | `${logfile}  ${marker}` | Wait for a per-quickstart readiness string in the captured `diagrid dev run` output — an extra agent-family gate layered on top of `Wait Until Apps Connected`, not a replacement for it, and empty (`READY_MARKERS = ()`) where the README names no marker. The marker is a property of the agent framework, not the language; read it out of the README's "wait until..." prose (see `references/agent-quickstart.md`). |
| `Wait Until Catalyst Attached` | `${logfile}  ${marker}` | Wait for an **inbound** request from Catalyst in the captured `diagrid dev run` output — the point at which Catalyst has attached to the app and workflow calls stop hanging. Agent-family only, driven by `${qs}[catalyst_probe_markers]`, and empty for a quickstart whose marker has not been observed. Must run **before** the first documented request: a workflow call made inside the window hangs and never recovers (12 retries over 181s, measured 2026-08-27), so there is no retry shape that works. Passive on purpose — two active probes were tried and are vacuous. See "The Catalyst-attach gate" in `references/agent-quickstart.md`. |
...
| `Wait Until Apps Healthy` | `${qs}` | Poll every `(port, path)` pair in `${qs}[health_probes]` until it answers 200, with `${READINESS_TIMEOUT}` overall and a 3s poll interval. The only readiness gate for quickstarts (like `workflow`, `state`) that emit no connection marker at all. **The path is per quickstart and you must confirm the app serves it** — see "Probe a path the app really serves" in `references/agent-quickstart.md`. |
| `Health Check Returns 200` | `${port}  ${path}=/` | Internal single-shot check `Wait Until Apps Healthy` polls with. The `/` default is what the canonical apps serve (verified in all sixteen implementations); it is not a safe default for an agent app. |
...
| `POST And Expect The App To Exit` | `${port}  ${path}  ${payload}  ${timeout}=120` | POST and assert the request did NOT complete, because the app killed its own process while handling it. For a quickstart whose documented trigger crashes on purpose — `agents/microsoft-dotnet`: "Call `step_two_compare` — crashes before completing (process exits)". Three outcomes, two of which fail: the connection drops -> PASS; **any status code -> FAIL**, because that is the crash having silently stopped happening (Program.cs shipped with the crash line commented out, so a fresh clone's demo did nothing); **a timeout -> FAIL, distinctly**, because a hang is Catalyst's attach window, not this crash, and a suite with no attach gate must not report one as a pass. Driven by a request's `expect: "connection-dropped"`. Tested in `resources/tests/crash_assertion.robot` against `crashing_server.py`. |
