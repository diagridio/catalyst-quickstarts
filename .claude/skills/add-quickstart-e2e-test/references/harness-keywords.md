...
| `POST And Expect` | `${port}  ${path}  ${payload}  ${status}  ${expected_body}=${NONE}` | POST JSON and assert the status code, plus (if given) the **exact** parsed response body. Canonical by default — response bodies are deterministic there. For an agent-family suite use `POST And Expect Field` instead, **unless the response is produced before the model runs** and is therefore byte-identical run to run: `agents/langgraph/enterprise-identity` asserts the exact `401 {"error": "oauth.missing_token"}` its middleware returns, which no model output can reach. If the response embeds model output, `POST And Expect Field` is still the rule. |
