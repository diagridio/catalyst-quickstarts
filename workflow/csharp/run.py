...
CSHARP_INSTRUCTIONS = """
.NET 8+ SDK must be installed to run this script. Full instructions can
be found on the .NET web site:
...
def check_dotnet_installed():
    with yaspin(text="Checking .NET SDK installation...") as spinner:
        version_check = run_command("dotnet --version")
        if version_check is None:
            error(spinner, "Error: .NET SDK must be installed to run this script.")

        try:
            version_parts = version_check.strip().split('.')
            major_version = int(version_parts[0])
            if major_version < 8:
                error(spinner, f"Error: .NET SDK version 8 or higher is required. Found version: {version_check.strip()}")
        except (IndexError, ValueError):
            error(spinner, f"Error: Unable to determine .NET SDK version from output: {version_check.strip()}")
...
def main():
    ...
    check_appid_status(project_name, "workflow-app")
